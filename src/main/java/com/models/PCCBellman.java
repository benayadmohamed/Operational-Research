...
public class PCCBellman {
	...
	public void CalculePccNonOrienteOuOriente(int k) {
		int i;
		for (Sommet sommet : SousEnsembleP0) {
			System.out.print(" " + sommet.getNom() + " " + sommet.getLongueurPcc());
		}
		System.out.println();
		...
		if (k <= SousEnsembleP1.size() && compareSousEnsembleP1EtSousEnsembleP0()) {
			...
			for (Sommet x : SousEnsembleS) {
				...
				FindMinSommetDansS(i);
			}

			CalculePccNonOrienteOuOriente(k + 1);
	...
	public void FindMinSommetDansS(int x) {
		int i;
		Double min = SousEnsembleP0.get(x).getLongueurPcc();
		...
		for (Arret arret : list) {
			...
			if (SousEnsembleP0.get(i).getLongueurPcc() != null) {
				...
				if (min == null) {
					...
					min = SousEnsembleP0.get(i).getLongueurPcc() + arret.getLongueur();
				} else if (min > SousEnsembleP0.get(i).getLongueurPcc() + arret.getLongueur()) {
					SousEnsembleP1.get(x).setLongueurPcc(SousEnsembleP0.get(i).getLongueurPcc() + arret.getLongueur());
					SousEnsembleP1.get(x).setPrédécesseurPcc(SousEnsembleP0.get(i).getId());
					min = SousEnsembleP0.get(i).getLongueurPcc() + arret.getLongueur();
				}
